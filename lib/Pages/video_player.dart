...
class VideoPageState extends State<VideoPage>
  ...
  Widget build(BuildContext context) {
    ...
    videoProvider.updateControlsVisibility(false, 1.0);
    ...
    Widget miniPlayer(controller, source) {
      Widget player() {
        return GestureDetector(
            ...
            child: source == VideoSource.youtube
                ? YoutubePlayer(
                    controller:
                        (controller as YoutubeControllerWrapper).controller)
                : VideoPlayer(
                    (controller as LocalControllerWrapper).controller));
      }
      ...
      return Container(
        ...
        height: Globals.size(context).height * 0.15,
    ...
    Widget maximizedPlayer(mainPage, controller, source) {
      Widget player(controller, source) {
        return GestureDetector(
          onTap: () {
            showControls();
          },
      ...
      Widget playbackSpeed(controller, source) {
                      ...
                      child: ValueListenableBuilder<double>(
                        ...
                        builder: (context, opacity, _) {
                          return DropdownButton<double>(
                            ...
                            items: const [
                              DropdownMenuItem(
                                  value: 0.25, child: Text("0.25")),
                              DropdownMenuItem(
                                  value: 0.50, child: Text("0.50")),
                              DropdownMenuItem(
                                  value: 0.75, child: Text("0.75")),
                              DropdownMenuItem(
                                  value: 1.00, child: Text("1.00")),
                              DropdownMenuItem(
                                  value: 1.25, child: Text("1.25")),
                              DropdownMenuItem(
                                  value: 1.50, child: Text("1.50")),
                              DropdownMenuItem(
                                  value: 1.75, child: Text("1.75")),
                              DropdownMenuItem(
                                  value: 2.00, child: Text("2.00")),
                            ],
                          );
                        },
                      ),
      ...
      Widget mainControls(controller, source) {
        return ValueListenableBuilder<bool>(
            valueListenable: videoProvider.showControlsNotifier,
      ...
      Widget durationSlider(controller) {
        return ValueListenableBuilder(
            valueListenable: videoProvider.showControlsNotifier,
            builder: (context, isVisible, _) {
              return !isVisible
            ...
            Expanded(
              child: Stack(
                children: [
                  ...
                  title(),
                  mainControls(controller, source),
                  playbackSpeed(controller, source),
                  durationSlider(controller),
                ],
              ),
            )
          ],
                    ...
                    return source == null
                        ...
                        : ValueListenableBuilder(
                            ...
                            builder: (context, mainPage, _) {
                              ...
                              return !mainPage
                                  ? miniPlayer(controller, source)
                                  : maximizedPlayer(
                                      mainPage, controller, source);
                            });
                  });
        });
  }
